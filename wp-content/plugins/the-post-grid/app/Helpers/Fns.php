...
class Fns {
	...
	public static function update_post_views_count( $post_id ) {
		...
		if ( false === ( $visited ) ) {
			...
			set_transient( $key, $value, 10 ); // store the unique key, Post ID & IP address for 12 hours if it does not exist.
	...
	public static function get_render_data_set( $data, $total_pages, $posts_per_page, $_prefix, $is_gutenberg = '' ) {
		...
		$data_set = [
			...
			'prefix'                       => $_prefix,
			'grid_column'                  => isset( $data[ $_prefix . '_column' ] ) ? $data[ $_prefix . '_column' ] : '0',
			...
			'title_position'               => isset( $data['title_position'] ) ? $data['title_position'] : 'default',
			'category_style'               => isset($data['category_style']) ? $data['category_style'] : '',
	...
	static function get_el_thumb_cat( $data, $class = 'cat-over-image' ) {
		...
		if ( in_array( $data['layout'], [ 'grid-layout4', 'slider-layout3', 'grid_hover-layout11' ] ) && 'default' === $data['category_position'] ) {
	...
	public static function get_post_thumbnail( $pID, $data, $link_start, $link_end, $offset_size = false ) {
		...
		} else if ( in_array( $data['layout'], [ 'grid-layout4', 'grid_hover-layout11', 'slider-layout3' ] ) && 'default' === $data['category_position'] ) {
			$thumb_cat_condition = true;
		}
            ...
            <div class="read-more">
				...
				if ( 'yes' == $data['show_btn_icon'] && 'left' == $data['readmore_icon_position'] ) {
					if ( $type === 'elementor' ) {
						...
						echo "<i class='left-icon " . esc_attr( $data['readmore_btn_icon'] ) . "'></i>";
					}
				}
				...
				if ( 'yes' == $data['show_btn_icon'] && 'right' == $data['readmore_icon_position'] ) {
					if ( $type === 'elementor' ) {
						...
						echo "<i class='right-icon " . esc_attr( $data['readmore_btn_icon'] ) . "'></i>";
					}
				}
